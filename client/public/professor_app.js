...
console.log("Requesting initial roster...");
...
socket.on('roster_update', (data) => {
    ...
    rosterDiv.innerHTML = '';
    console.log(data.student_attendance_info)
    if (data.student_attendance_info && data.student_attendance_info.length > 0) {
        data.student_attendance_info.forEach(student => {
            ...
            const isOnline = student.check_in_status.includes('Online');
...
socket.on('connect_error', () => {
    console.error("Gateway connection failed. Is express_middleware running?");
});